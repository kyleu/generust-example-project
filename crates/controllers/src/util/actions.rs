...
pub(crate) fn err(ctx: &RequestContext, e: &generust_example_project_core::Error) -> HttpResponse {
  let content = match generust_example_project_templates::error::exception(ctx, e) {
    Ok(c) => c.into_string(),
...
pub(crate) fn not_found(session: Session, cfg: Data<AppConfig>, path: Path<String>, req: HttpRequest) -> HttpResponse {
  let ctx = crate::req_context(&session, &cfg, &req, "index");
  let content = match generust_example_project_templates::error::not_found(&ctx, &path) {
    Ok(c) => c.into_string(),
    Err(e) => format!("A critical system error has occurred: {}", e.to_string())
  };
