...
const FLASH_KEY: &str = "generust-example-project-flash";
const SESSION_ID_KEY: &str = "generust-example-project-session";

/// Creates a [RequestContext](generust_example_project_service::RequestContext) from an HTTP request
pub fn req_context(session: &Session, cfg: &Data<AppConfig>, action_key: &'static str) -> RequestContext {
  ...
  let user_profile = generust_example_project_service::profile::load(app_cfg.files(), user_id);
  let logger = app_cfg
    .root_logger()
